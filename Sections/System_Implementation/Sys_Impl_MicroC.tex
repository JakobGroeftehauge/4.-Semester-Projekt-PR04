...
The microcontroller have build in hardware that enables it to transmit and receive data using SPI. The microcontroller is set up as a master, as described in section \ref{subsec:SystemImplemtationFPGA}. To select a desired slave, the a 4-bit address is used. Four general purpose input output pins on the microcontroller are used for selecting a slave, while a hardware controlled pin connected to SPI, is used to enable the slaves. The microcontroller uses a bit rate of \SI{13.3}{\mega bit} when communicating with the slaves on the FPGA. 
