...
On figure \ref{fig:SPILogicSimple} an overview of the SPI module is shown. The module contains seven internal slaves. Four slaves hold the information used for the controllers in the microcontroller. Two holds the PWM provided by the microcontroller, needed to control the voltage supplied to the motors. The last slave is used for the Protocol module. The microcontroller is the master device controlling the slaves. The microcontrolle can access the slaves via a 4 -  

In order for the microcontroller to access a specfic slave, it asks for an address

the microcontroller can select one of four slaves by  wires. 



The timing of the SPI connection is regulated by the microcontroller, why the FPGA is only responsible for storing the sampled values in the 
