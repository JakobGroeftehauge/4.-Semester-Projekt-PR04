...
The overall structure of the controller task can be seen on figure \ref{fig:PIDControllerFlowchart}. When the feedback data is received, the error between the reference value and feedback value is calculated. As mentioned in section \ref{subsec:SystemImplemtationFPGA}, because of possible variance in velocity feedback due to the imprecise separation of edges in the encoder, a second order lowpass Finite Impulse Response (IIR) filter is applied to the error. 

Since the error ideally would consist of a DC frequency component, the filter was designed using a cutoff frequency at $0.5\,\mathrm{Hz}$. The filter transfer function used is:
\begin{equation}
